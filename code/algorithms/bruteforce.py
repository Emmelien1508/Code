...
class BruteForce():
    ...
    def run(self):
        ...
        for possible_tour in possible_tours:
            ...
            self.sort_scenarios(list(possible_tour))
            ...
            self.all_tours[possible_tour] = self.stsp_length
            j += 1

        self.tour = min(self.all_tours, key=self.all_tours.get)
        self.stsp_length = self.all_tours[self.tour]
    ...
    def reset(self):
        """
        Resets all values and attributes.
        """
        self.stsp_length = 0
